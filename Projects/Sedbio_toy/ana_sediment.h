...
# elif defined ESTUARY_TEST2
      DO j=JstrT,JendT
        ...
        END DO
      END DO
# elif defined SEDBED_TOY || defined SEDBIO_TOY
      DO j=JstrT,JendT
        DO i=IstrT,IendT
          bottom(i,j,isd50)=0.0005_r8
          bottom(i,j,idens)=2650.0_r8
...
!
          DO k=1,Nbed
            bed(i,j,k,iaged)=time(ng)
            bed(i,j,k,ithck)=0.0005_r8
            bed(i,j,k,iporo)=0.90_r8
            bed(i,j,k,idiff)=1.0d-10
            DO ised=1,NST
              bed_frac(i,j,k,ised)=1.0_r8/REAL(NST,r8)
            END DO
#  if defined SEDBIO_COUP
             bed(i,j,k,iboxy)=0.0_r8                                   &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibno3)=0.0_r8                                   &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibnh4)=10.0_r8                                  &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibodu)=1.0_r8                                   &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
#  endif
          END DO
          bed(i,j,Nbed,ithck)=10.0_r8
!
!  Set exposed sediment layer properties.
!
          bottom(i,j,irlen)=0.0_r8
...
!            DO ised=1,NST
!              bed_frac(i,j,k,ised)=1.0_r8/REAL(NST,r8)
              bed_frac(i,j,k,1)=0.4_r8-0.015_r8
              bed_frac(i,j,k,2)=0.4_r8-0.015_r8
              bed_frac(i,j,k,3)=0.2_r8
              bed_frac(i,j,k,4)=0.015_r8
              bed_frac(i,j,k,5)=0.015_r8
!            END DO
#  if defined SEDBIO_COUP
              bed(i,j,k,iboxy)=5.0_r8                                   &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibno3)=0.0_r8                                   &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibnh4)=1000.0_r8                                &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
              bed(i,j,k,ibodu)=100.0_r8                                  &
     &            *bed(i,j,k,ithck)*bed(i,j,k,iporo)
#  endif
          END DO
          bed(i,j,Nbed,ithck)=10.0_r8
#  if defined SEDBIO_COUP
          bed(i,j,Nbed,ibnh4)=3000.0_r8                                 &
     &            *bed(i,j,Nbed,ithck)*bed(i,j,Nbed,iporo)
          bed(i,j,Nbed,ibodu)=300.0_r8                                  &
     &            *bed(i,j,Nbed,ithck)*bed(i,j,Nbed,iporo)
          DO k=11,Nbed
            bed(i,j,k,iboxy)=0.0_r8
          END DO 
